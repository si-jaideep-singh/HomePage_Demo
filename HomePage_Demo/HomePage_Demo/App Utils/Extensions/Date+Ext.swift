...
//
//  Created by Abhijeet Gawde on 22/02/23.
//

import Foundation

extension Date {
    func toString(format: LSGDateFormats) -> String? {
    ...
    func getTimeDifference(firstDate: String?, secondDate: String?, dateformat: LSGDateFormats) -> TimeInterval {
    ...
    func dateFormatWithSuffix(date: String, dateFormat: LSGDateFormats) -> String {
    ...
    func convertTimezoneOf(time: String?,from currentTimeZone: String, to requiredTimeZone: String) -> String {
        ...
        var temp = dateFormatter.date(from: time ?? .blank) ?? Date()
